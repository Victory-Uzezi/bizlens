...
function categorise(desc) {
  ...
  // Self top-ups / internal transfers (Nigerian pattern: received from own name)
  if (/received from (omovrah|omovra|victory uzezi|victory omovrah)/.test(d)) return "Self Top-up";

  // Church / Tithe / Offerings
  if (/church|chapel|christ|christian|ministry|tithe|offering|parish|cathedral|centre ado|new reality/.test(d)) return "Church / Tithe";

  // POS Withdrawals (very common in Nigeria)
  if (/pos transfer|pos-transfer|send to pos/.test(d))  return "POS Withdrawal";

  // Food & Dining
  if (/eateri|eatries|eatry|restaurant|kitchen|food|canteen|cafe|bukka|suya|pepper soup|joyful/.test(d)) return "Food & Dining";

  // Telecoms — data, airtime
  if (/buy data|data bundle|top up airtime|airtime|mtn|airtel|glo |9mobile|ussd/.test(d)) return "Telecoms";

  // Bank charges
  if (/electronic money transfer levy|transfer levy|stamp duty|vat charge|maintenance fee|sms charge|ussd charge/.test(d)) return "Bank Charges";

  // Education
  if (/school|academy|college|university|tuition|education|lesson|tutorial/.test(d)) return "Education";

  // Shopping / Markets
  if (/store|market|shop|mall|supermarket|ventures|enterprises|nig limited|provision|merchandise|frozen food|agro/.test(d)) return "Shopping";

  // Healthcare
  if (/pharmacy|hospital|clinic|medical|health|doctor|dental|chemist/.test(d)) return "Healthcare";

  // Transport
  if (/uber|bolt|taxi|transport|bus|fuel|petrol|logistics/.test(d)) return "Transport";

  // Rent / Housing
  if (/rent|house|apartment|estate|property|landlord|lease/.test(d)) return "Rent / Housing";

  // Subscriptions
  if (/netflix|spotify|dstv|gotv|showmax|prime|subscription|apple/.test(d)) return "Subscriptions";

  // Savings / Investments
  if (/savings|investment|piggybank|cowrywise|risevest|stash/.test(d)) return "Savings";

  // Income / Salary
  if (/salary|payroll|wages|income|stipend/.test(d)) return "Income";

  // Received money (credits from other people)
  if (/^received from/.test(d)) return "Money Received";

  // Personal transfers to people (send to [name])
  if (/^send to/.test(d)) return "Personal Transfers";
...
function PersonalBankTab({ sharedData, setSharedData }) {
  ...
  const processFile = async (file) => {
    ...
    try {
      let transactions = [], headers = [], extraCols = [];

      if (ext === "pdf") {
        // ── PDF: send to AI, get structured JSON back ──────────────────────
...
Return ONLY the JSON array. Start your response with [ and end with ]`;
        ...
        // ── Excel / CSV: use SheetJS ───────────────────────────────────────
        const result = await parseExcel(file);
        headers = result.headers;
        extraCols = result.extraCols;
        transactions = result.transactions.map(t => ({ ...t, category: categorise(t.desc) }));
      }

      if (transactions.length === 0) throw new Error("No transactions found.");
      const analytics = deriveAnalytics(transactions, extraCols);
      const parsed = { transactions, analytics, extraCols, fileName: file.name, rawHeaders: headers };
      setData(parsed);
      autoInsight(transactions, analytics, extraCols, file.name);
    } catch (err) {
  ...
  return (
    <div>
      <div style={{ marginBottom:22 }}>
        ...
        <p style={{ fontSize:12, color:"#6B7A8D", margin:"0 0 20px" }}>Choose a file type to upload — BizLens will analyse your statement instantly</p>
        ...
        <div>
          ...
          {parsing && (
            <div style={{ ...card, textAlign:"center", padding:"32px", marginBottom:16 }}>
              ...
              <p style={{ color:"#E8C67A", fontSize:14 }}>Reading your file — this may take 15–30 seconds for PDFs…</p>
            </div>
          )}
      ...
      {data && (
        <div>
          ...
          <div style={{ ...card, marginBottom:18 }}>
            ...
            <p style={{ fontSize:11, color:"#6B7A8D", margin:"0 0 14px" }}>BizLens has full context of your uploaded data including all detected columns</p>
  ...
  return (
    <div>
      <div style={{ marginBottom:22 }}>
        ...
        <p style={{ fontSize:12, color:"#6B7A8D" }}>Upload CSV, Excel, or PDF bank statements — BizLens analyses them instantly</p>
      </div>
      ...
      <div style={{ ...card, marginBottom:24 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:16 }}>
          {[
            ...
            { step:"03", title:"Get AI insights", desc:"BizLens reads your data, categorises transactions, and gives you personalised financial insights" },
          ].map(s => (
      ...
      {pdfName && (
        <div style={{ ...card, marginTop:24 }}>
          ...
          {pdfLoading
            ...
            : <div>
                <p style={{ fontSize:11, color:"#6B7A8D", marginBottom:12 }}>Text extracted — BizLens can now analyse this. Go to the <strong style={{ color:"#E8C67A", cursor:"pointer" }} onClick={()=>onNavigate("ask")}>Ask AI tab</strong> to ask questions about it.</p>
        ...
        {tab==="ask" && sharedData && (
          <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 130px)" }}>
            <div style={{ marginBottom:18 }}>
              ...
              <p style={{ fontSize:12, color:"#6B7A8D" }}>{sharedData.fileName}</p>
            </div>
