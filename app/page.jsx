...
function PersonalBankTab() {
  const [data, setData]           = useState(null); // { transactions, analytics, extraCols, fileName, rawHeaders }
  ...
  return (
    <div>
      <div style={{ marginBottom:22 }}>
        ...
        <p style={{ fontSize:12, color:"#6B7A8D", margin:0 }}>Upload any bank statement Excel or CSV — Claude adapts to your file's columns automatically</p>
      </div>
      ...
      {!data && (
        <div
          ...
          style={{ ...card, borderStyle:"dashed", borderColor: dragging?"#E8C67A":"rgba(255,255,255,0.12)", background: dragging?"rgba(232,198,122,0.05)":"rgba(255,255,255,0.02)", textAlign:"center", padding:"56px 24px", transition:"all 0.2s", cursor:"pointer" }}
        >
          ...
          <p style={{ color:"#F0EEE8", fontSize:16, fontWeight:500, marginBottom:8 }}>
            {parsing ? "Reading your file…" : "Drop your bank statement here"}
          </p>
          <p style={{ color:"#6B7A8D", fontSize:13, marginBottom:20 }}>
            Works with any bank export — Excel (.xlsx, .xls) or CSV<br/>
            <span style={{ color:"#4B5563", fontSize:11 }}>Columns are detected automatically regardless of format</span>
          </p>
          {!parsing && (
            <button style={{ background:"linear-gradient(135deg,#E8C67A,#C9963A)", border:"none", borderRadius:8, padding:"10px 28px", fontSize:14, color:"#0C0F14", fontWeight:600, cursor:"pointer" }}>
              Browse Files
            </button>
          )}
          {parsing && <div style={{ color:"#E8C67A", fontSize:13, marginTop:8 }}>Parsing your statement…</div>}
          {parseError && <p style={{ color:"#F87171", fontSize:13, marginTop:14 }}>{parseError}</p>}
        </div>
      )}
...
export default function BizLens() {
  ...
  const [msgs, setMsgs]     = useState([{ role:"assistant", content:"Hi! I'm BizLens AI. Ask me anything about your business metrics, or head to the Banking tab to upload any personal bank statement for a full AI-powered analysis." }]);
  ...
  const send = async () => {
    ...
    const sys = `You are BizLens AI, a business analyst assistant. Business data: Revenue Feb $33,900 | Expenses $15,400 | Margin 54.6% | Forecast March $36,500. Be concise and specific.`;
    try {
      const reply = await askClaude(next.filter(m=>m.role==="user").map(m=>({role:"user",content:m.content})), sys);
      setMsgs(prev=>[...prev,{role:"assistant",content:reply}]);
    } catch(e) {
      setMsgs(prev=>[...prev,{role:"assistant",content:`Error: ${e.message}. Check ANTHROPIC_API_KEY in .env.local`}]);
    }
    setLoad(false);
  };
  ...
  const tabs = [
    ...
    { id:"banking",  label:"Banking",   icon:"⊛", badge:"PERSONAL" },
